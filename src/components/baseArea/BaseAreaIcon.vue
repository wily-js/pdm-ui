...
const skipBread = (item) => {
    ...
    breadpath(url)
    router.push({
        path: "/index/baseDocumentArea" + url
    })
}
// 面包屑返回按钮
const back = () => {
    ...
    for (let index = 0; index < pathList.value.length - 1; index++) {
        if (pathList.value[index].name !== "基础文档区") {
...
const list = (item) => {
    ...
    axios.get("/api/baseDocArea/list?base=" + item).then((resp) => {
...
// 下载
const download = () => {
    window.location.href = "/api/baseDocArea/download?path=" + encodeURIComponent(menuItem.value.path)
}
...
const remove = () => {
    ...
    ElMessageBox.confirm(
        ...
        loading.value = true
        axios.delete("/api/baseDocArea/remove?path=" + encodeURIComponent(menuItem.value.path)).then((resp) => {
...
const getAttribute = () => {
    loading.value = true
    axios.get('/api/baseDocArea/attr?path=' + encodeURIComponent(menuItem.value.path)).then((resp) => {
        loading.value = false
...
const rename = () => {
    ElMessageBox.prompt('请输入文件/文件夹名称', '重命名', {
        ...
        loading.value = true
        axios.post("/api/baseDocArea/move", form).then((resp) => {
            loading.value = false
...
const newFileFolder = () => {
    ...
    // axios.post("/api/baseDocArea/mkdir", JSON.stringify(base)).then((resp) => {
...
const drop = (val) => {
    ...
    if (dragItem.value != -1) {
        ...
        axios.post("/api/baseDocArea/move", moveOrCopyUrl.value).then((resp) => {
...
const uploadFiles = (form) => {
    ...
    axios.post("/api/baseDocArea/upload", form, {
...
const fileSearch = () => {
    ...
    let info = {}
    let url = "/api/baseDocArea/search?"
    if (path.value !== "") {
...
const loadNode = (node, resolve) => {
    ...
    setTimeout(() => {
        ...
        axios.get("/api/baseDocArea/list?base=" + base + "&type=dir").then((resp) => {
            resolve(resp.data)
        }).catch((err) => {
            ElMessage.error({ message: err.response.data, duration: 1000, showClose: true })
        })
...
const fileCopy = () => {
    ...
    axios.post("/api/baseDocArea/copy", moveOrCopyUrl.value).then((resp) => {
