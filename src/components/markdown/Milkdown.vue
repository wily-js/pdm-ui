...
const uploadAPI = (file) => {
  ...
  if (codeType.value == "stage") {
    formData.append("stageId", id.value)
    formData.append("file", file)
    formData.append("projectId", projectId)
    loading.value = true
    axios.post("/api/project/stage/assert", formData).then((resp) => {
  ...
  else if (codeType.value == "doc") {
    formData.append("docId", id.value)
    formData.append("file", file)
    formData.append("projectId", projectId)
    loading.value = true
    axios.post("/api/doc/assert", formData).then((resp) => {
      if (resp.data.docType == "file") {
        const insertContent = "[" + file.name + "]" + "(" + resp.data.uri + ")";
        getInstance().action(insert(insertContent))
      }
