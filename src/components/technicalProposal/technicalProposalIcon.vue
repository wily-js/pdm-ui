<template>
    <div class="know-body" v-contextmenu:contextmenu @mousedown.right="clearMenuItem" @drop="drop" v-loading="loading"
        ...
        <div class="know-content" style="display:flex; flex-wrap: wrap;">
            ...
            <div class="fileItem" style="margin:58px;" v-for="item, index in fileData" :key="index"
                @mousedown.right.stop="getMenuItem(item)" draggable="true" :id="index" @dragstart="drag" @dragover="over"
                @dragleave="leave">
                <div>
                    <div v-if="item.type === 'dir'" @click="skip(item)">
                        <img src="../../assets/fileType/folder.svg" style="height:48px;" draggable="false" :id="index" />
                    </div>
    ...
    <v-contextmenu ref="contextmenu">
        ...
        <v-contextmenu-item @click="moveOrCopy" v-if="menuItem.type !== ''">复制/移动
        </v-contextmenu-item>
        <v-contextmenu-item @click="rename" v-if="menuItem.type !== ''">重命名
        </v-contextmenu-item>
        <v-contextmenu-item @click="getAttribute" v-if="menuItem.type !== ''">属性</v-contextmenu-item>
        <v-contextmenu-item style="color:red;" @click="remove" v-if="menuItem.type !== ''">
...
const skipBread = (item) => {
    ...
    breadpath(url)
    router.push({
        path: "/index/technicalProposal" + url
    })
}
// 面包屑返回按钮
const back = () => {
    ...
    for (let index = 0; index < pathList.value.length - 1; index++) {
        if (pathList.value[index].name !== "技术方案") {
...
const list = (item) => {
    ...
    axios.get("/api/technicalProposal/list?base=" + item).then((resp) => {
...
// 下载
const download = () => {
    window.location.href = "/api/technicalProposal/download?path=" + encodeURIComponent(menuItem.value.path)
}
...
const remove = () => {
    ...
    ElMessageBox.confirm(
        ...
        loading.value = true
        axios.delete("/api/technicalProposal/remove?path=" + encodeURIComponent(menuItem.value.path)).then((resp) => {
...
const getAttribute = () => {
    loading.value = true
    axios.get('/api/technicalProposal/attr?path=' + encodeURIComponent(menuItem.value.path)).then((resp) => {
        loading.value = false
...
const rename = () => {
    ElMessageBox.prompt('请输入文件/文件夹名称', '重命名', {
        ...
        loading.value = true
        axios.post("/api/technicalProposal/move", form).then((resp) => {
            loading.value = false
...
const newFileFolder = () => {
    ...
    // axios.post("/api/technicalProposal/mkdir", JSON.stringify(base)).then((resp) => {
...
const drop = (val) => {
    ...
    if (dragItem.value != -1) {
        ...
        axios.post("/api/technicalProposal/move", moveOrCopyUrl.value).then((resp) => {
...
const uploadFiles = (form) => {
    ...
    axios.post("/api/technicalProposal/upload", form, {
...
const fileSearch = () => {
    ...
    let info = {}
    let url = "/api/technicalProposal/search?"
    if (path.value !== "") {
...
const loadNode = (node, resolve) => {
    ...
    setTimeout(() => {
        ...
        axios.get("/api/technicalProposal/list?base=" + base + "&type=dir").then((resp) => {
            resolve(resp.data)
        }).catch((err) => {
            ElMessage.error({ message: err.response.data, duration: 1000, showClose: true })
        })
...
const fileCopy = () => {
    ...
    axios.post("/api/technicalProposal/copy", moveOrCopyUrl.value).then((resp) => {
