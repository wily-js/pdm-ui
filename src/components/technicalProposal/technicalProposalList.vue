<template>
            ...
            <el-table-column prop="item" label="名称" width="800">
                <template #default="scope">
                    <div class="icon" draggable="true" @dragstart="drag(scope.row)" :id="scope.$index"
                        ...
                        <img v-else-if="scope.row.type === 'dir'" src="../../assets/fileType/folder.svg" :id="scope.$index"
                            style="height:24px;" draggable="false" />
    ...
    <v-contextmenu ref="contextmenu" :disabled="menuFlag">
        ...
        <v-contextmenu-item style="color:red;" @click="remove" v-if="menuItem.type !== ''">删除
        </v-contextmenu-item>
...
const skip = (val) => {
    if (val.type == "dir") {
        ...
        router.push({
            path: "/index/technicalProposal" + path.value
        })
    }
...
const getList = (item) => {
    loading.value = true
    axios.get("/api/baseDocArea/list?base=" + encodeURIComponent(item)).then((resp) => {
