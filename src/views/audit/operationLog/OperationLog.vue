<template>
    <div class="top">
        ...
        <el-select v-model="user" placeholder="用户名" :loading="loading" filterable remote :reserve-keyword="false"
            ...
            <div v-loading="loading" element-loading-background="rgba(255, 255, 255, 0.8)" element-loading-text="数据加载中...">
                <el-option style="height:45px;" v-for="item in persion" :key="item.id" :value="item.id" :label="item.name">
    ...
    <el-dialog v-model="dialog" :close-on-click-modal="false" :show-close="false" title="操作参数" width="30%">
        <el-descriptions direction="horizontal" :column="1" border>
            <el-descriptions-item v-for="(item, key) in paramForm" :key="key" :label="key" label-align="center"
                align="center">
                {{ item }}
